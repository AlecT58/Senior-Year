...
namespace FOL_IFC
{
    class FOL_IFC
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < sentences.Count; i++)
            {
                ...
                {
                    if(sentences[i].isRuleInSentence(aFact))
                    {
    ...
    class Sentence
    {
        ...
        public Boolean isRuleInSentence(String aRule)
        {
            String ruleName = aRule.Split('(')[0];

            foreach (Atom anAtom in leftHandAtoms)
            {
                if(anAtom.name.Equals(ruleName))
                {
                    return true;
                }
            }
